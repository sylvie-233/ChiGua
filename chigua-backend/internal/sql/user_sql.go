...
// UserInsert 插入用户
const UserInsert = `INSERT INTO users (username, password, nickname, avatar, created_at, update_at) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id`

// UserSelectByUP 按用户名密码查询用户
const UserSelectByUP = `SELECT id, username, password, nickname, avatar, created_at, update_at FROM users WHERE username = $1 AND password = $2`

// UserSelectByID 按ID查询用户
const UserSelectByID = `SELECT id, username, password, nickname, avatar, created_at, update_at FROM users WHERE id = $1`
...
const UserSelectAll = `SELECT id, username, nickname, avatar, created_at, update_at FROM users ORDER BY created_at DESC LIMIT $1 OFFSET $2`
...
SELECT id, username, nickname, avatar, created_at, update_at
FROM users
WHERE username ILIKE '%' || $1 || '%' OR nickname ILIKE '%' || $1 || '%'
...
const AdminUserUpdate = `UPDATE users SET nickname = COALESCE($1, nickname), avatar = COALESCE($2, avatar), update_at = $3 WHERE id = $4`
